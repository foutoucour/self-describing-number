...
from model import is_length_even, get_binomials, is_binomial_describing, is_enough_binomials
...
def process_task(index, task: list[str], progress=False) -> list[str]:
    results = []
    if progress:
        task = track(task, description=f"Thread {index}...", transient=True, total=len(task), refresh_per_second=10, update_period=0.5)
    for i in task:
        if is_valid(str(i)):
...
@click.option("number_executor", "--number-executor", "-n", type=click.INT, default=os.cpu_count(),
              help="Number of executors to use", show_default=True)
@click.option("progress", "--progress/--no-progress", default=False, help="Show progress bar", show_default=True)
def main(number: int, number_executor: int, progress: bool):
    command(number, number_executor, progress)
