...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            "23322114", True, id="23322114: same as 14233221, but different order"
        ),
        ...
        pytest.param(
            "183110", False, id="311018: not self-describing, we don't count 3s"
        ),
        pytest.param(
            "666666", False, id="666666: only 6s, should be only one binomial"
        ),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param("22", True, id="22"),
        pytest.param("666666", True, id="666666: not self-describing"),
        pytest.param("", True, id="empty string"),
    ],
)
def test_are_binomials_ordered(value: str, expected: bool):
    binomials = get_binomials(value)
    assert are_binomials_ordered(binomials) == expected
